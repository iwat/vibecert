...
func setupFlagsForCreateCertificateParams(cmd *cobra.Command, params *application.CreateCertificateRequest) error {
	...
	cmd.Flags().IntVar(&params.SubjectKeyID, "key-id", params.SubjectKeyID, "ID of existing key to be signed by this certificate, leave out to create a new one")
...
func printCertificateTree(certs []*application.CertificateNode, prefix string) {
	for i, cert := range certs {
		...
		fmt.Printf("%s%s %s\n", prefix, marker, cert)
		if cert.Key != nil {
			idLength := len(fmt.Sprintf("%d", cert.Certificates[len(cert.Certificates)-1].ID)) + 5
